...
export default function PinCard({ pin, onClick, className, showDetails = false, priority = false }: PinCardProps) {
  ...
  const content = (
    ...
    >
      ...
      />
      {showDetails && pin.uploader && !isImageLoading && ( // Only show details if image loaded
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-between p-3 text-white">
          <div className="flex justify-end items-start">
            {/* Future: More options button for save, hide, report */}
             <Button variant="secondary" size="icon" className="bg-white/20 hover:bg-white/30 text-white rounded-full h-8 w-8 opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => {e.stopPropagation(); /* handle more options */}}>
          ...
          <div className="flex items-center justify-between text-xs">
            <Link href={`/u/${pin.uploader.username}`} className="flex items-center gap-1.5 group/uploaderinfo hover:underline" onClick={(e) => e.stopPropagation()}>
              ...
              <span className="truncate font-medium">{pin.uploader.full_name || pin.uploader.username}</span>
            </Link>
