...
import { SlidersHorizontal, Plus, Edit2, Share2, Search as SearchIcon, Users, Settings } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  }, [toast, router, activeTab]); // Added activeTab
 ...
 useEffect(() => {
   ...
   useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && hasMorePins && !isLoadingPins && userProfile?.id && activeTab === 'created') {
  ...
  }, [loadMorePins, hasMorePins, isLoadingPins, userProfile?.id, pinsPage, activeTab]);
    ...
    <div className="flex-1 flex flex-col animate-fade-in-up pt-8">
      <main className="flex-grow container mx-auto px-4 pb-6">
        <div className="flex flex-col items-center text-center">
          ...
          {userProfile.username && <p className="text-muted-foreground text-sm sm:text-base">@{userProfile.username}</p>}
          ...
          {userProfile.website && (
            <a 
              ...
              target="_blank" rel="noopener noreferrer" className="mt-1 text-sm text-primary hover:underline"
            >
        ...
        <div className="flex-1 container mx-auto px-0 sm:px-4 pb-8 mt-8">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <div className="sticky top-[calc(var(--header-height)+var(--header-height))] sm:top-[var(--header-height)] bg-background/90 backdrop-blur-md z-10 py-3 -mx-2 sm:-mx-4 px-2 sm:px-4 border-b mb-6">
              <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                      ...
                      <Button variant="ghost" size="icon" className="rounded-full text-muted-foreground hover:bg-secondary hover:text-foreground" onClick={() => router.push('/create')}>
            ...
            <TabsContent value="created">
              ...
              {isLoadingPins && pins.length === 0 && ( 
                <div className="masonry-grid px-grid-gap md:px-0 mt-grid-gap pt-6">
                  {[...Array(6)].map((_, i) => (
                    <div key={`skeleton-profile-created-${i}`} className="break-inside-avoid mb-grid-gap">
                      <Skeleton className={`w-full h-[${200 + Math.floor(Math.random() * 150)}px] rounded-2xl bg-muted/80`} />
                    </div>
                  ))}
                </div>
               ...
               {!hasMorePins && pins.length === 0 && !isLoadingPins && (
                <div className="text-center py-16">
                  ...
                  <p className="text-muted-foreground mt-1">Start creating and sharing your ideas!</p>
                  <Button className="mt-6 rounded-full px-6" size="lg" onClick={() => router.push('/create')}>
                    Create Pin
                  </Button>
                </div>
              )}
            ...
            <TabsContent value="saved">
              <div className="text-center py-16">
                ...
                <p className="text-muted-foreground mt-1">Explore and save pins you love to see them here.</p>
                 <Button className="mt-6 rounded-full px-6" size="lg" onClick={() => router.push('/')}>
                    Explore Pins
                  </Button>
              </div>
            </TabsContent>
          ...
          {!hasMorePins && pins.length > 0 && activeTab === 'created' && (
            <p className="text-center text-muted-foreground font-medium py-10 text-lg">✨ You've explored all your created pins! ✨</p>
          )}
        </div>
      </main>
    </div>
