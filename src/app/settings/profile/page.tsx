...
import { Camera, Save, UserCircle, Mail, Globe, Loader2, Building2, MapPin } from 'lucide-react';
...
export default function ProfileSettingsPage() {
  ...
  useEffect(() => {
    const getUserAndProfile = async () => {
      ...
      if (session?.user) {
        ...
        if (error && error !== 'PGRST116') { // PGRST116 means no row found, which is fine for new users before profile is fully set up.
          toast({ variant: 'destructive', title: 'Error fetching profile', description: error || 'Could not load your profile data.' });
        } else if (profile) {
          setUserData(profile);
        ...
        toast({ variant: 'destructive', title: 'Not Authenticated', description: 'Please log in to edit your profile.' });
        router.push('/login');
      }
      setIsLoading(false);
    };
  ...
  const handleUsernameBlur = async () => {
    if (userData.username && userData.username.trim() !== initialUsername) {
      ...
      const { available, error } = await checkUsernameAvailability(userData.username.trim());
      if (error) {
      ...
      setIsSaving(false);
    } else if (userData.username && userData.username.trim() === initialUsername) {
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          <section className="p-6 bg-card rounded-2xl shadow-card">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6">
              ...
              <div>
                <Button type="button" variant="outline" className="rounded-full focus-ring" onClick={() => document.getElementById('avatarUpload')?.click()} disabled={isSaving}>
                  Upload New Picture
                </Button>
                <p className="text-xs text-muted-foreground mt-2">PNG, JPG, GIF, WEBP. Max 2MB.</p>
              </div>
            </div>
          </section>
          ...
          <section className="p-6 bg-card rounded-2xl shadow-card">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
              <div>
                <Label htmlFor="full_name" className="text-sm font-medium text-foreground/90">Full Name</Label>
                 <div className="relative mt-1">
                  ...
                  <Input id="full_name" name="full_name" value={userData.full_name || ''} onChange={handleChange} className="pl-10 h-11 focus-ring" placeholder="E.g. Jane Doe" disabled={isSaving}/>
                </div>
              </div>
              <div>
              ...
              <div className="md:col-span-2">
                ...
                <Textarea id="bio" name="bio" value={userData.bio || ''} onChange={handleChange} rows={3} className="mt-1 focus-ring" placeholder="Tell us a little about yourself..." disabled={isSaving} maxLength={160} />
                <p className="text-xs text-muted-foreground mt-1">Appears on your profile. Max 160 characters.</p>
              </div>
              <div>
                <Label htmlFor="website" className="text-sm font-medium text-foreground/90">Website</Label>
                 <div className="relative mt-1">
                  ...
                  <Input id="website" name="website" type="url" value={userData.website || ''} onChange={handleChange} className="pl-10 h-11 focus-ring" placeholder="https://yourwebsite.com" disabled={isSaving}/>
                </div>
              </div>
            </div>
          </section>
            ...
            <h2 className="text-xl font-semibold mb-6 text-foreground">Account Information</h2>
             <div>
                ...
                <div className="relative mt-1">
                  ...
                  <Input id="email" name="email" type="email" value={currentUserEmail || 'Loading...'} className="pl-10 h-11 bg-muted/50 border-muted/30 cursor-not-allowed" placeholder="your.email@example.com" disabled readOnly />
                </div>
